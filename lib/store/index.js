...
class FSTStore {
  constructor (indice, config, options = {}) {
    ...
    this.paths = {
      nodes: path.join(__dirname, '..', '..', 'indexes', `${this.indice}.nodes.json`),
      records: path.join(__dirname, '..', '..', 'indexes', `${this.indice}.records.json`),
      facets: path.join(__dirname, '..', '..', 'indexes', `${this.indice}.facets.json`)
    }
